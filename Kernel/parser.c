...
//	command=strtok(req," ");
	if(!strcmp(command,"ADD"))
		request.command=ADD_PARSER;
	else if(!strcmp(command,"RUN"))
		request.command=RUN_PARSER;
	else if(!strcmp(command,"SELECT"))
		request.command=SELECT_PARSER;
	else if(!strcmp(command,"INSERT"))
		request.command=INSERT_PARSER;
	else if(!strcmp(command,"CREATE"))
		request.command=CREATE_PARSER;
	else if(!strcmp(command,"DESCRIBE"))
		request.command=DESCRIBE_PARSER;
	else if(!strcmp(command,"DROP"))
		request.command=DROP_PARSER;
	else if(!strcmp(command,"JOURNAL"))
		request.command=JOURNAL_PARSER;
	else if(!strcmp(command,"METRICS"))
		request.command=METRICS_PARSER;
	else if(!strcmp(command,"SALIR"))
		request.command=SALIR_PARSER;
	else {
		request.command=INVALID_COMMAND_PARSER;
	}
