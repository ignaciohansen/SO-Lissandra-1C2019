...
//    infoPagina* infoPag = malloc(sizeof(infoPag));
	...
	for (i = 0; i < 2; i++) {
		...
		printf("DATOS GRAL: %d  %d  %f\n", pagina_obtenida->nroPosicion,
				pagina_obtenida->key, pagina_obtenida->timestamp);
...
log_info(log_memoria,
		"[JOURNAL] ENVIO LA CANTIDAD EXACTA DE CARACTERES QUE LE VOY A ENVIAR");

log_info(log_memoria, "[JOURNAL] TAMAÑO ENVIADO");

log_info(log_memoria,
		"[JOURNAL] Lisandra responde que se puede enviar todo, procedo a hacerlo");
...
log_info(log_memoria,
		"[CREANDO SEGMENTO] Creando segmento para la tabla '%s' asociando al nro de pagina '%d'",
...
if (tablaSegmentos == NULL) {
	log_info(log_memoria,
			"[CREANDO SEGMENTO] ES EL PRIMER SEGMENTO EN SER CREADO '%s'",
			pathNombreTabla);
	...
	log_info(log_memoria,
			"[CREANDO SEGMENTO] CREANDO OTRO SEGMENTO A LA TABLA SEGMENTOS '%s'",
			pathNombreTabla);
...
void segmento_asociar_nueva_pagina(segmento* unSegmento,
	pagina_referenciada* ref) {
mutexBloquear(&mutex_segmento_modificando);
unSegmento->cantPaginasAsociadas += 1;
int cantidad = unSegmento->cantPaginasAsociadas;
log_info(log_memoria,
		"[ASOCIANDO NUEVA PAGINA A SEGMENTO] Por asociar la pagina '%d' al segmento '%s'",
...
log_info(log_memoria,
		"[ASOCIANDO NUEVA PAGINA A SEGMENTO] Segmento '%s' se le ha asignado con exito el nro de pagina %d",
...
log_info(log_memoria,
		"[ELIMINANDO PAGINA A SEGMENTO] Sacando nro de pagina '%d' del segmento '%s'",
...
log_info(log_memoria,
		"[SEGMENTO BUSCAR TABLA Y KEY] Por buscar KEY '%d' y PATH '%s'", key,
		nombreTabla);
...
while (aux != NULL) {
	...
	if (strcmp(nombreTabla, aux->path_tabla)) {
		int nroDePagina;
		log_info(log_memoria,
				"[SEGMENTO BUSCAR TABLA Y KEY] SEGMENTO ENCONTRADO de '%s', voy a buscar la key",
				nombreTabla);
		nroDePagina = buscarEntreTodasLasTablaPaginasLaKey(aux->paginasAsocida,
				key);
		//	mutexDesbloquear(&mutex_pagina_referenciada_aux2);
		if (nroDePagina > ERROR) {
			log_info(log_memoria,
					"[SEGMENTO BUSCAR TABLA Y KEY] SEGMENTO ENCONTRADO de '%s', KEY encontrada '%d'",
					nombreTabla, key);
			if (comandoInsert) {
...
informacion = accederYObtenerInfoDePaginaEnPosicion(nroDePaginaAIr,
		informacion);
...
log_info(log_memoria,
		"[OBTENIENDO DATOS] Empiezo a obtener datos de pagina '%d'", i);
mutexBloquear(&mutex_memoria);
memcpy(informacion, bloque_memoria + i * (sizeof(pagina) + max_valor_key),
		sizeof(pagina) + max_valor_key);
...
log_info(log_memoria, "[SELECT] Por acceder a la memoria a la posicion '%d'",
		posicion);
...
log_info(log_memoria,
		"[OBTENIENDO DATOS] Incremento el valor de Acceso para la pagina con key '%d'",
		pag->key);
incrementarAccesoDeKey(pag->key);

memcpy(bloque_memoria + posicion * (sizeof(pagina) + max_valor_key), pag,
		sizeof(pagina));
log_info(log_memoria,
		"[asignarNuevaTablaAPosicionLibre] Cantidad de accesos de la pagina NRO '%d' de la key '%d' ACTUALIZADA",
		posicion, pag->key);
...
log_info(log_memoria,
		"[ACCEDIENDO A DATOS] Por acceder a la memoria a la posicion '%d'",
		posicion);
//	mutexBloquear(&mutex_tabla_pagina_en_modificacion);
info = obtenerInfoDePagina(posicion, info);
log_info(log_memoria,
		"[ACCEDIENDO A DATOS] Datos obtenidos de la posicion '%d'", posicion);
//	mutexDesbloquear(&mutex_tabla_pagina_en_modificacion);
...
pagina_referenciada* ref;
log_info(log_memoria,
		"[INCREMENTAR ACCESO EN 1] Entrando, empiezo a buscar la key '%d'",
		keyBuscada);
while (seg_aux != NULL) {
	...
	while (ref != NULL) {
		if (ref->key == keyBuscada) {
			ref->vecesAccedido += 1;
			log_info(log_memoria,
					"[INCREMENTAR ACCESO EN 1] Se ha incrementado el nro de accesos a la pagina. SALIMOS");
...
pagina_referenciada* tabla_pagina_crear(u_int16_t key, char* valor,
bool flag_modificado) {
char valor_string[max_valor_key];
log_info(log_memoria,
		"[Crear Tabla y pagina] En crear Tabla de pagina y pagina nueva porque no estan con la key %d",
		key);
...
void asignarNuevaPaginaALaPosicion(int posLibre, pagina* pagina_nueva,
	char* valorAPoner) {
log_info(log_memoria,
		"[asignarNuevaTablaAPosicionLibre] Por guardar los datos de el valor y la nueva pagina en memoria");
//	char stringValor[max_valor_key];
//	strcpy(stringValor, valorAPoner);

mutexBloquear(&mutex_memoria);
mutexBloquear(&mutex_bitmap);
memcpy(bloque_memoria + posLibre * (sizeof(pagina) + max_valor_key),
		pagina_nueva, sizeof(pagina));
log_info(log_memoria, "[asignarNuevaTablaAPosicionLibre] Pagina guardada");

memcpy(
		bloque_memoria + posLibre * (sizeof(pagina) + max_valor_key)
				+ sizeof(pagina) - 1, valorAPoner, max_valor_key);
log_info(log_memoria,
		"[asignarNuevaTablaAPosicionLibre] El valor de la pagina fue guardada, actualizo el BITMAP ocupando la posicion '%d'",
		posLibre);
//	free(stringValor);
bitmapOcuparBit(bitmap, posLibre);

log_info(log_memoria,
		"[asignarNuevaTablaAPosicionLibre] Se han guardado exitosamente los datos\nAhora mismo hay '%d' espacios libres",
		cantPaginasDisponibles);
pagina* pagNew = malloc(sizeof(pagina));
char valorString[max_valor_key];
memcpy(pagNew, bloque_memoria + posLibre * (sizeof(pagina) + max_valor_key),
		sizeof(pagina));
memcpy(valorString,
		bloque_memoria + posLibre * (sizeof(pagina) + max_valor_key)
				+ sizeof(pagina) - 1, max_valor_key);

log_info(log_memoria,
		"[asignarNuevaTablaAPosicionLibre]POSICION %d\nVALORES PUESTOS: KEY|VALUE|TIMESTAMP %d|%s|%f\nVALORES PUESTOS: KEY|VALUE|TIMESTAMP %d|%s|%f\n",
...
log_info(log_memoria, "[CREANDO PAGINA Y VALOR]KEY|VALOR|TIMESTAMP: %d|%f|%s",
		key, algo, valor);
log_info(log_memoria,
		"[CREANDO PAGINA Y VALOR] Pagina creada y tambien su valor");
return pag;
}

//void buscarKeyPorTablaPagina(tabla_pagina* tabla_pag, pagina* pag, bool aniadirSiNoHay, bool modificar) {
int buscarKeyPorTablaPagina(pagina_referenciada* tabla_pagina_auxx,
...
while (aux_tabla_paginas2 != NULL) {
	if (aux_tabla_paginas2->key == keyBuscada) {
		log_info(log_memoria,
				"[Buscar Key Por Tabla PAGINA] SE ENCONTRO LA PAGINA PARA LA KEY EN LA POSICION PARA LA KEY|POSICION %d|%d",
...
log_info(log_memoria,
		"[Buscar Key Por Tabla PAGINA] NO SE ENCONTRO LA PAGINA PARA LA KEY EN LA POSICION PARA LA KEY%d",
		keyBuscada);
...
int buscarEntreTodasLasTablaPaginasLaKey(
	pagina_referenciada* tablasAsociadasASegmento, u_int16_t keyBuscada) {
...
int buscarEntreLosSegmentosLaPosicionXNombreTablaYKey(char* nombreTabla,
	u_int16_t keyBuscada, segmento** segmentoBuscado, int* nroDePagina) {
log_info(log_memoria,
...
if (seg_aux != NULL) {
	...
	log_info(log_memoria,
			...
			nombreTabla);
	return buscarEntreTodasLasTablaPaginasLaKey(seg_aux->paginasAsocida,
			keyBuscada);
}
...
if (strlen(valorAPoner) > max_valor_key) {
	log_error(log_memoria,
			"[INSERT] El valor VALUE '%s' es mayor que el max value KEY\nMOTIVO: %d Mayor que %d",
...
log_info(log_memoria,
		"[INSERT] Me pongo a buscar el segmento y la tabla en base a '%s' y '%d'",
		nombreTabla, keyBuscada);
int posicionAIr = buscarEntreLosSegmentosLaPosicionXNombreTablaYKey(nombreTabla,
		keyBuscada, &segmentoBuscado, &nroPosicion);

if (posicionAIr == -1) {
	log_info(log_memoria,
			"[INSERT] NO se encontro la posicion a donde debo ir");
	//CASO B, verifico si se encontro el segmento, caso contrario debo tambien crearlo
	if (segmentoBuscado == NULL) {
		log_info(log_memoria,
				"[INSERT] Tampoco se encontro que existe un segmento asociado a la tabla '%s'/nProcedo a crear el segmento, tabla de pagina y alojar la pagina en memoria",
				nombreTabla);
		...
		log_info(log_memoria,
				...
				ref->nropagina, ref->key);
		segmentoBuscado = segmento_crear(nombreTabla, ref);
		log_info(log_memoria,
				"[INSERT] SEGMENTO Creado para la tabla '%s' /nComo es el primer segmento TABLA SEGMENTOS apuntara este elemento",
				segmentoBuscado->path_tabla);
		tablaSegmentos = segmentoBuscado;
		log_info(log_memoria,
				"[INSERT] Se ha creado un segmento para la tabla '%s'",
				nombreTabla);
	} else {
		log_info(log_memoria,
				"[INSERT] Se encontro un segmento asociado a la tabla '%s'",
				segmentoBuscado->path_tabla);
		...
		log_info(log_memoria,
				"[INSERT] Se ha creado una tabla de pagina, el nro de su posicion es '%d'",
				ref->nropagina);
		//		segmentoAgregarNroTabla(&segmentoBuscado, nroTablaCreada);
		log_info(log_memoria,
				"[INSERT] Se ha actualizado segmento para la tabla '%s'",
				nombreTabla);
	}
} else {
	/*
	 ...
	 */
	log_info(log_memoria,
			"[INSERT] Existe el segmento '%s' y la pagina que referencia la key (%d) que es NRO '%d'\nProcedo a poner el nuevo valor que es '%s'",
			nombreTabla, keyBuscada, posicionAIr, valorAPoner);

	modificarValoresDeTablaYMemoriaAsociadasAKEY(posicionAIr, valorAPoner,
			nroPosicion);
}
return 1;
}
...
memcpy(aux, bloque_memoria + posAIr * (sizeof(pagina) + max_valor_key),
		sizeof(pagina));
printf("1 hecho\n");
memcpy(valorString,
		bloque_memoria + posAIr * (sizeof(pagina) + max_valor_key)
				+ sizeof(pagina) - 1, max_valor_key);
log_info(log_memoria,
		"[Modificar valor pagina] Para la pagina con key '%d';  TIMESTAMP '%d'; VALOR '%s'",
		aux->key, aux->timestamp, valorString);

aux->timestamp = timestamp();
log_info(log_memoria,
		"[Modificar valor pagina] Incremento el acceso a la pagina '%d' de la key '%d'",
		posAIr, aux->key);
...
log_info(log_memoria,
		"[Modificar valor pagina] Pagina modificada con key '%d' VALORES NUEVOS;  TIMESTAMP '%d'; VALOR '%s'",
		aux->key, aux->timestamp, valorString);

log_info(log_memoria,
		"[MOdificar valor pagina] Guardando los datos actualizados la pagina con key: %d",
		aux->key);
memcpy(bloque_memoria + posAIr * (sizeof(aux) + max_valor_key), aux,
		sizeof(aux));
log_info(log_memoria, "[MOdificar valor pagina] key: '%d', VALOR NUEVO: %s",
		aux->key, valorString);
...
log_info(log_memoria,
		"[Modificar valor tabla pagina] Procedo a buscar Segmento y Tabla de pagina que contiene a key: %d",
		keyBuscada);
...
while (nuevoSeg != NULL) {
	...
	while (ref != NULL) {
		...
		if (ref->key == keyBuscada) {
			log_info(log_memoria,
					"[Modificar valor tabla pagina] Key '%d' encontrada en el segmento y nro de pagina '%s'|'%d'",
...
mutexBloquear(&ACCIONLRU);
log_info(log_memoria,
		"[LRU] Comenzando el LRU, empiezo a buscar la pagina a reemplazar");
...
for (i = 0; i < cantPaginasTotales; i++) {
	...
	log_info(log_memoria, "[LRU] Obtuve datos de la pagina y tambien KEY %d|%d",
			i, aux_pag_devolver->key);
	log_info(log_memoria,
			"[LRU] Busco la key entre los segmentos y tabla de paginas");
	if (auxSeg != NULL) {
		//
			...
			if (ref->key == aux_pag_devolver->key) {
				if (ref->flag == true) {
					if (minHastaAHora == -1) {
						...
						candidatoAQuitar = i;
						log_info(log_memoria,
								"[LRU Actualizo candidato] Pagina|MAX Cantidad de veces accedidas hasta ahora: %d|%d",
								candidatoAQuitar, minHastaAHora);
						//		ref = NULL;
					} else {
...
if (candidatoAQuitar < 0) {
	log_info(log_memoria,
			"[LRU sin candidato] NO hay nada que se puede quitar, por lo tanto se fuerza un JOURNAL");
	JOURNAL();
	log_info(log_memoria,
			"[LRU sin candidato] JOURNAL HECHO, lo asigno a la primera posicion");
	paginaCreada->nroPosicion = 0;
	asignarNuevaPaginaALaPosicion(0, paginaCreada, valor);
	mutexDesbloquear(&JOURNALHecho);
} else {
	log_info(log_memoria,
			"[LRU con candidato] Pondre la pagina en la posicion a reemplazar: %d",
			candidatoAQuitar);
...
log_info(log_memoria, "[LIBERAR SEGMENTO] Liberando segmento de la tabla '%s'",
		nombreTabla);
segmento* seg_aux;
...
segmento* aux;
log_info(log_memoria,
		"[LIBERAR SEGMENTO] BLOQUEO EL MUTEX mutex_segmento_en_modificacion PARA QUE NADIE PUEDA AGREGAR MIENTRAS");
mutexBloquear(&mutex_segmento_en_modificacion);
log_info(log_memoria,
		"[LIBERAR SEGMENTO] EMPIEZO A LIBERAR TODOS LOS SEGMENTOS");
while (tablaSegmentos != NULL) {
	aux = tablaSegmentos->siguienteSegmento;
	log_info(log_memoria, "[LIBERAR SEGMENTO] LIBERANDO SEGMENTO DE TABLA '%s'",
...
log_info(log_memoria,
		"[LIBERAR SEGMENTO] DESBLOQUEO EL MUTEX mutex_segmento_en_modificacion PARA MODIFICACIONES");
}

void liberar_toda_tabla_paginas(pagina_referenciada* pag) {
pagina_referenciada* aux;
log_info(log_memoria, "[LIBERAR TABLA DE PAGINAS] Libero pagina nro '%d'",
		pag->nropagina);
while (pag != NULL) {
	aux = pag->sig;
	log_info(log_memoria, "[LIBERAR TABLA DE PAGINAS] Libero pagina nro '%d'",
...
log_info(log_memoria,
		"[LIBERAR PAGINAS] BLOQUEO EL MUTEX mutex_tabla_pagina_en_modificacion");
mutexBloquear(&mutex_tabla_pagina_en_modificacion);
int i;

for (i = 0; i < cantPaginasTotales; i++) {
	memcpy(bloque_memoria + i * (sizeof(pagina) + max_valor_key), pag,
			sizeof(pagina) + max_valor_key);
}
...
log_info(log_memoria,
		"[LIBERANDO TABLA DE PAGINAS] Por liberar la tabla de paginas de todo segmento");
...
while (segaux != NULL) {
	...
	log_info(log_memoria,
			"[LIBERANDO TABLA DE PAGINAS] Liberando tablas de paginas del segmento '%s'",
			segaux->path_tabla);
	while (ref != NULL) {
		otro = ref->sig;
		log_info(log_memoria,
				"[LIBERANDO TABLA DE PAGINAS] Pagina nro '%d' con key '%d' LIBERADO",
				ref->nropagina, ref->key);
		free(ref);
		ref = otro;
	}
...
log_info(log_memoria,
		"[LIBERAR PAGINAS] PONGO EL SEMAFORO cantPaginasDisponibles = cantPaginasTotales");
cantPaginasDisponibles = cantPaginasTotales;
log_info(log_memoria,
		"[LIBERAR PAGINAS] DESBLOQUEO EL MUTEX mutex_tabla_pagina_en_modificacion");
mutexDesbloquear(&mutex_tabla_pagina_en_modificacion);
...
if (tablaSegmentos == NULL) {
	log_info(log_memoria,
			"[BUSCANDO SEGMENTO X KEY] No se ha inicializado la tabla de segmentos, devuelvo NULL");
	return NULL;
}
segmento* todoSegmento = tablaSegmentos;
char* nombreEnSegmento;
log_info(log_memoria,
		"[BUSCANDO KEY] Buscando el segmento referido para la tabla '%s'",
		nombreTabla);
while (todoSegmento != NULL) {
	...
	log_info(log_memoria,
			"[BUSCANDO KEY] Nombre buscado | NOmbre obtenido '%s'|'%s'",
			nombreTabla, nombreEnSegmento);

	if (stringIguales(nombreEnSegmento, nombreTabla)) {
		//SE ENCONTRO EL SEGMENTO BUSCADO
		log_info(log_memoria,
				"[BUSCANDO SEGMENTO X NOMBRETABLA] Se encontro el segmento buscado");
		return todoSegmento;
	}
	todoSegmento = todoSegmento->siguienteSegmento;
}
log_info(log_memoria,
		"[BUSCANDO SEGMENTO X KEY] NO se encontro el segmento buscado, devuelvo NULL");
return NULL;
}
...
log_info(log_memoria,
		"[BITMAP] Verifico si tiene espacio libre del total de paginas que son: '%d'",
		cantPaginasTotales);
mutexBloquear(&mutex_bitmap);
for (i = 0; i < cantPaginasTotales; i++) {
	...
	if (!bitmapBitOcupado(bitmap, i)) {
		...
		log_info(log_memoria,
				"[BITMAP] La posicion '%d' esta NO libre pero verifico si tiene la misma key la cual es '%d'",
				i, key);
		pag = selectObtenerDatos(i);
		log_info(log_memoria,
				"[BITMAP] Obtuve la pagina de la posicion '%d' y tiene key '%d' y busco la key '%d'",
				i, pag->key, key);

		if (pag->key == key) {
			log_info(log_memoria,
					"[BITMAP] La posicion '%d' contiene a la key '%d' por lo tanto la devuelvo",
					i, key);
			mutexDesbloquear(&mutex_bitmap);
			return i;
		}
